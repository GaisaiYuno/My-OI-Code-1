...
void push_up(int pos){
	if(tree[pos].cnt>=2){
		tree[pos].sum1=tree[pos].sum2=all[tree[pos].r+1]-all[tree[pos].l];
	}else if(tree[pos].cnt==1){
		tree[pos].sum1=all[tree[pos].r+1]-all[tree[pos].l];
		if(tree[pos].l==tree[pos].r) tree[pos].sum2=0;
		else tree[pos].sum2=tree[pos*2].sum1+tree[pos*2+1].sum1;
	}else{
		if(tree[pos].l==tree[pos].r) tree[pos].sum1=tree[pos].sum2=0; 
		else {
			tree[pos].sum1=tree[pos*2].sum1+tree[pos*2+1].sum1;
			tree[pos].sum2=tree[pos*2].sum2+tree[pos*2+1].sum2;
		}
	}
}
void build(int l,int r,int pos){
     tree[pos].l=l;
     tree[pos].r=r;
     tree[pos].cnt=tree[pos].sum1=tree[pos].sum2=0;
     if(l==r) return;
     int mid=(l+r)/2;
     build(l,mid,pos*2);
