...
int main(){
	...
	lca_init();
	for(int i=1;i<=m;i++){
		ans[G[i].index]=tlen+(long long)G[i].len-(long long)lca_query(G[i].from,G[i].to);
//		printf("debug: %I64d %I64d %I64d\n",tlen,(long long)G[i].len,(long long)tlen+G[i].len-ans[G[i].index]);
	}
	for(int i=1;i<=m;i++){
		printf("%I64d\n",ans[i]);
	}
