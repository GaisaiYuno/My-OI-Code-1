...
int bfs(){  
    ...
    queue[0].y=1;
    queue[0].step=0;
    if(keys[1][1]>0) queue[0].key=(queue[0].key|(1<<(keys[1][1]-1)));//起点有钥匙的特判 
    else queue[0].key=0;
        ...
        for(int i=0;i<4;i++){
            ...
            if(judge(now.x,now.y,nex.x,nex.y)){
                if(keys[nex.x][nex.y]>0){
                    tmp=keys[nex.x][nex.y]-1;
                    nex.key=(nex.key|(1<<tmp)); 
                }
                if(door[now.x][now.y][nex.x][nex.y]>0){
