...
long long query(int to_l,int to_r,int l,int r,int pos,int que_type){
	...
	if(to_l<=mid){
	    ...
	    if(que_type==2) ans2=min(ans2,(long long)query(to_l,to_r,l,mid,pos<<1,2));
	    if(que_type==3) ans3=max(ans3,(long long)query(to_l,to_r,l,mid,pos<<1,3));
	}
	if(to_r>mid){
		if(que_type==1) ans1+=query(to_l,to_r,mid+1,r,pos<<1|1,1);
	    if(que_type==2) ans2=min(ans2,(long long)query(to_l,to_r,mid+1,r,pos<<1|1,2));
	    if(que_type==3) ans3=max(ans3,(long long)query(to_l,to_r,mid+1,r,pos<<1|1,3));
	}
