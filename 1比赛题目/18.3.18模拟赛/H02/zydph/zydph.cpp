...
void push_down(int pos,int ln,int rn){
	if(tree[pos].mark2){
		tree[pos<<1].mark1=tree[pos<<1].mark1=0;
	...
	if(tree[pos].mark1){
		tree[pos<<1].mark1+=tree[pos].mark1;
		tree[pos<<1|1].mark1+=tree[pos].mark1;
		...
		tree[pos].mark1=0;
		tree[pos<<1].mark2=tree[pos<<1].mark2=0;
	}
}
...
long long query(int to_l,int to_r,int l,int r,int pos,int que_type){
	...
	push_down(pos,mid-l+1,r-mid);
	if(to_l<=mid){
	    if(que_type==1) ans1+=query(to_l,to_r,l,mid,pos<<1,que_type);
	    if(que_type==2) ans2=min(ans2,(int)query(to_l,to_r,l,mid,pos<<1,que_type));
	    if(que_type==3) ans3=max(ans3,(int)query(to_l,to_r,l,mid,pos<<1,que_type));
	}
	if(to_r>mid){
		if(que_type==1) ans1+=query(to_l,to_r,mid+1,r,pos<<1|1,que_type);
	    if(que_type==2) ans2=min(ans2,(int)query(to_l,to_r,mid+1,r,pos<<1|1,que_type));
	    if(que_type==3) ans3=max(ans3,(int)query(to_l,to_r,mid+1,r,pos<<1|1,que_type));
	}
