...
inline void insert_left(int pos,int x){
	q[id[pos]].push_front(x-mark[id[pos]]);
}
...
inline void insert_right(int pos,int x){
	q[id[pos]].push_back(x-mark[id[pos]]);
}
...
//	if(q[id[x]].tail!=NULL) q[id[x]].tail->nex=q[id[y]].head;
//	if(q[id[y]].head!=NULL) q[id[y]].head->pre=q[id[x]].tail;
//	q[id[x]].tail=q[id[y]].tail;
//	q[id[y]].head=q[id[y]].tail=NULL;
}
